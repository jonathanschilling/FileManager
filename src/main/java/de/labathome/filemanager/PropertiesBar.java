...
/**
 * a bar for the detailed view mode which contains some columns like file name,
 * file size, etc.
 */
public class PropertiesBar {
	...
	/**
	 * get the most right line of the bar (used for consecutive painting of the
	 * columns)
	 *
	 * @param getMostRight - if true, get the most right already painted edge, else
	 *                     get the total will-be right edge
	...
	public void setPreferences(Preferences p) {
		...
		int i = 0;
		while (myProperties.size() < Preferences.columnsCaptions.size()) {
			myProperties.add(new PropertiesItem(Preferences.columnsCaptions.get(i)));
	 ...
	 * @param columnIndex - which index the current column has (used to determine
	 *                    the column caption)
