...
//        Any number of rows and 2 columns
		...
		nameField = new JTextField(myPrefs.columnsCaptions.get(0), 20);
		nameField.addActionListener(this);
		
		sizeField = new JTextField(myPrefs.columnsCaptions.get(1), 20);
		sizeField.addActionListener(this);
		
		ownerField = new JTextField(myPrefs.columnsCaptions.get(2), 20);
		ownerField.addActionListener(this);
		
		lastChangeField = new JTextField(myPrefs.columnsCaptions.get(3));
		lastChangeField.addActionListener(this);
		
		permissionsField = new JTextField(myPrefs.columnsCaptions.get(4));
	...
	private boolean saveToFile(String filename) {
		...
		try {
			...
			System.out.println("Tried to save preferences file " + filename
					+ ": " + e.toString());
			JOptionPane.showMessageDialog(new JFrame(),
					"<html><center>Could not save preferences file.<br/>" + e.getMessage() + "</center></html>", "Error",
	...
	private boolean loadFromFile(String filename) {
		...
		try {
			...
			JOptionPane.showMessageDialog(new JFrame(),
					"<html><center>Could not load preferences file.<br/>" + e.getMessage() + "</center></html>", "Error",
					JOptionPane.ERROR_MESSAGE);
			System.out.println(e.getMessage());
			System.out.println("Tried to load preferences file " + filename
					+ ": " + e.getMessage());
		}
		return success;
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		// select background color
			...
			myPrefs.backgroundColor = JColorChooser.showDialog(this, "Choose Background Color", myPrefs.backgroundColor);
			bgColorButton.setBackground(myPrefs.backgroundColor);
		}
		...
		// select background color for unselected items
			...
			myPrefs.unselectedColor = JColorChooser.showDialog(this, "Choose Unselected Color", myPrefs.unselectedColor);
			unselectedColorButton.setBackground(myPrefs.unselectedColor);
		}
		...
		// select color for PropertiesItems' captions
			...
			myPrefs.selectRectColor = JColorChooser.showDialog(this, "Choose Select Rect Color", myPrefs.selectRectColor);
			selectRectColorButton.setBackground(myPrefs.selectRectColor);
		}
		
		// select caption for Name column
		else if (e.getSource() == nameField) {
			myPrefs.columnsCaptions.set(0, e.getActionCommand());
		}
		
		// select caption for Size column
		else if (e.getSource() == sizeField) {
			myPrefs.columnsCaptions.set(1, e.getActionCommand());
		}
		
		// select caption for Owner column
		else if (e.getSource() == ownerField) {
			myPrefs.columnsCaptions.set(2, e.getActionCommand());
		}
		
		// select caption for Last Change column
		else if (e.getSource() == lastChangeField) {
			myPrefs.columnsCaptions.set(3, e.getActionCommand());
		}
		
		// select caption for Permissions column
		else if (e.getSource() == permissionsField) {
			myPrefs.columnsCaptions.set(4, e.getActionCommand());
		} 
